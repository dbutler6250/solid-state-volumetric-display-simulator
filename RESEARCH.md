...
PIEZO-DEFINED STRAIN STATES MATERIALLY IMPROVE ADDRESSABILITY in the bounded first optical pass. BIASED STRAIN TROUGH PERFORMS BEST: the best trough case used `0.8 mm` width, `0.25 mm` transition width, `0.0015` background bias strain, and `0.0015` local reduction, giving target selectivity about `44.29` versus about `2.97` for the best positive window under the same coarse search.
